...
def eigen_QR_gram_schmidt(A,tolerance = 1e-6):
    A = np.array(A)
    A_copy = np.copy(A)
    Q,R = gram_schmidt_qr(A)
    A = np.matmul(R,Q)
    i=1
    while np.linalg.norm(A-A_copy)>tolerance:
        A_copy = np.copy(A)
        Q,R = gram_schmidt_qr(A)
        A = np.matmul(R,Q)
        i+=1
    return np.diag(A),i
def polyfit_matrices(xlist: list, ylist: list, sigma_list: list, degree: int):
...
def customfn_fit_matrices(xlist: list, ylist: list, sigma_list: list, custom_fns: callable, degree: int):
    ...
    for i in range(degree+1):
        for j in range(degree+1):
            A[i,j] = np.sum(custom_fns(xlist,i)*custom_fns(xlist,j)/sigma_list**2)
            
    B = np.zeros(degree+1)
    for i in range(degree+1):
        B[i] = np.sum(ylist*custom_fns(xlist,i)/sigma_list**2)
        
    return A, B

def polynomial_fit_customfn(xlist: list,ylist: list,sigma_list: list,custom_fns: callable, degree: int,tolerance =1e-6):

    A, B = customfn_fit_matrices(xlist,ylist,sigma_list,custom_fns,degree)
        
    # solving the system of equations
    coeff_list = np.linalg.solve(A,B)
    
    return coeff_list


def customfn_coeff(x_list: list, degree: int, custom_fns: callable, coeff_list: list):
    
    y_list = np.zeros(x_list.shape)
    for i in range(degree+1):
        y_list += coeff_list[i]*custom_fns(x_list,i)
        
    return y_list
            
def calc_chi_squared(x_list: list, y_list: list, sigma_list: list, y_fit: list):    
    
    chi_squared = np.sum(((y_list - y_fit)/sigma_list)**2)
